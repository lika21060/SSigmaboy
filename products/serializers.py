...
#no need for fields bc theyre automarically included with modelserializer
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    def validate_rating(self, value):
        """Rating must be between 1 and 5, as other numbers do not correspond to the actual rating system."""
        if value < 1 or value > 5:
            raise serializers.ValidationError("Rating must be between 1 and 5.")
        return value

    def create(self, validated_data):
        """Create a review while ensuring the correct product-user relationship."""
...
class FavoriteProductSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        """Prevent adding duplicate favorite products."""
        user = self.context['request'].user
